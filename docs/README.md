...
Phase 0은 끝났고(모델 6종 측정 · 차원 1024 확정), **R-2a(리랭커)까지 나왔다.**
지연 걱정은 해소됐고(e2e p95 573~626ms), 세 arm 모두 정확도가 올랐다.
**⚠️ 그런데 리랭커가 임베딩 모델 격차의 87%를 흡수했다** — Phase 2에서 모델 선택에
쓸 시간의 우선순위가 내려갔다. 상세는 [experiments.md](experiments.md) R-2a 판정.

다음은 **R-3(하이브리드 BM25+RRF)**가 유력하다. 리랭커의 천장이 벡터 검색의
Hit@20(A 0.693)으로 드러났고, 그걸 올리려면 후보 자체를 넓혀야 하기 때문이다.
...
- ~~⚠️ **새 걱정거리는 리랭커 지연**~~ **해소(2026-08-18).** 2초 안팎 추정이었으나
  실측 **e2e p95 573~626ms**. 568M 리랭커를 그대로 쓴다
